...
def plot_count_best(df_pivot_val, df_pivot_test, name):
    ...
    # Plot
    plt.figure(figsize=(12, 7))
    minValueIndex_val.plot(kind='bar', color='skyblue', label='Count of the lowest validation errors')
    minValueIndex_test.plot(kind='bar', width=0.3, color='darkblue', label='Count of the lowest test errors over all datasets')
    plt.legend()
    plt.xlabel("Method")
    plt.ylabel("Count of the lowest error")
...
def plot_boxplot_percentage_impr(baseline_col, df_pivot, name):
    ...
    plt.xlabel("Method")
    plt.ylabel("Distribution of the percentage error reduction\nof the " + score_type + " error")
    plt.xticks(rotation=90, ha="right")
    plt.tight_layout()
