...
def recursive_feature_addition(i, n_features_to_add, X, y, model, method, dataset_metadata, category_to_drop):
    ...
    start = time.time()
    X, y = predict_improvement(result_matrix, comparison_result_matrix, method, i, X, y)
    end = time.time()
...
def recursive_feature_addition_mfe(i, n_features_to_add, X, y, model, method, dataset_metadata, category_to_drop):
    ...
    # Predict and split again
    X, y = predict_improvement(result_matrix_copy, comparison_result_matrix_copy, "all", i, X, y)
    # Recurse
    recursive_feature_addition_mfe(i + 1, n_features_to_add, X, y, model, method, dataset_metadata, category_to_drop)
    return X, y


def predict_improvement(result_matrix, comparison_result_matrix, category_or_method, i, X_train, y_train):
...
def main(dataset_id):
    ...
    for method in methods:
        if method == "mfe":
            ...
            X_train, y_train = recursive_feature_addition_mfe(j, n_features_to_add, X_train, y_train, model, method, dataset_metadata, None)
            ...
            for i in range(len(categories)):
                ...
                X_train, y_train = recursive_feature_addition_mfe(j, n_features_to_add, X_train, y_train, model, method, dataset_metadata, categories[i])
                data = concat_data(X_train, y_train, X_test, y_test, "target")
                data.to_parquet("FE_" + str(dataset_id) + "_" + str(method) + "_" + category + "_CatBoost_recursion.parquet")
            ...
            X_train, y_train, X_test, y_test, dataset_metadata = get_openml_dataset_split_and_metadata(dataset_id)
            start = time.time()
            X_train, y_train = recursive_feature_addition(j, n_features_to_add, X_train, y_train, model, method, dataset_metadata, None)
            end = time.time()
            print("Time for creating Comparison Result Matrix: " + str(end - start))
            data = concat_data(X_train, y_train, X_test, y_test, "target")
            data.to_parquet("FE_" + str(dataset_id) + "_" + str(method) + "_" + category + "_CatBoost_recursion.parquet")
