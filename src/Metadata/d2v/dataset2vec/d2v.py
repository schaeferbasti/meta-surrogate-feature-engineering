...
parser.add_argument('--LookAhead', help='Implement lookahead on backend optimizer', type=str, choices=['True', 'False'],
                    default='False')
...
for epoch in range(epochs):
    ...
    if optimizer.iterations % 50 == 0:
        ...
        model.report()
        if np.abs(model.metrics['roc'] - best_auc) > 1e-3:        # if np.abs(model.metrics['roc'].result().numpy() - best_auc) > 1e-3:
            best_auc = auc_score
            early = 0
        else:
            early += 1
