...
from src.utils.run_models import get_model_score, run_autogluon_lgbm


def get_core_result_feature_generation(X_train, y_train, X_test, y_test, dataset_metadata, train_feature, test_feature, featurename, original_results):
...
def get_core_result_feature_selection(X_train, y_train, X_test, y_test, dataset_metadata, featurename, original_results):
    operator = "delete"
    print("Run Autogluon with new Feature")
...
def main(dataset):
    ...
    for feature1 in X_train_copy.columns:
        ...
        new_rows = get_core_result_feature_selection(X_train_reduced, y_train, X_test_reduced, y_test, dataset_metadata, featurename, original_results)
    ...
    for feature1 in X_train_copy.columns:
        for feature2 in X_train_copy.columns:
            for operator in binary_operators:
                ...
                new_rows = get_core_result_feature_generation(X_train, y_train, X_test, y_test, dataset_metadata, train_feature, test_feature, featurename, original_results)
                result_matrix = pd.concat([result_matrix, pd.DataFrame(new_rows)], ignore_index=True)
                result_matrix.to_parquet("Operator_Model_Feature_Matrix_Core" + str(dataset) + ".parquet")
    print("Unary Features")
    for feature1 in X_train_copy.columns:
